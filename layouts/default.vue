<template>
  <div class="min-h-screen bg-gradient-to-br from-purple-50 to-indigo-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <Header  />
      <MyLogin v-if="authStore.showLoginModal" @close="authStore.closeLoginModal()" />
      <main class="bg-white shadow-md rounded-lg my-1 p-">
        <NuxtPage />
      </main>
      <!-- <Footer class="py-4" /> -->
      <GradientFooter />
...
onMounted(() => {
  window.addEventListener('login-success', () => {
    console.log("default.vue 收到 login-success 回调，关闭登录弹窗")
    authStore.login()
  })
})
